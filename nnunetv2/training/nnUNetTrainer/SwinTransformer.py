...
class SwinTransformerWrapper(nn.Module):
    def __init__(
        ...
        if swin_weights_path is not None:
            swin_weights = torch.load(swin_weights_path, weights_only=False)["student"]
            # strip keys prefix
            swin_weights = OrderedDict({key.removeprefix("module.backbone.").removeprefix("module.head."): val for key, val in swin_weights.items()})
            # remove extra layers from ibot training
            extra_keys = swin_weights.keys() - self.swin.state_dict().keys()
            swin_weights = OrderedDict({k: v for k, v in swin_weights.items() if k not in extra_keys})
            self.swin.load_state_dict(swin_weights)
        if freeze_swin:
...
class SwinTransformerV3(SwinTransformerWrapper):
    ...
    def forward(self, x, normalize=True, return_all_tokens=None, mask=None):
        swin_output = self.swin(x, normalize, return_all_tokens, mask)
...
class SwinTransformerV4(SwinTransformerWrapper):
    ...
    def forward(self, x, *args, **kwargs):
        ...
        for i in range(len(self.upscale)):
            upscaled = self.upscale[i](swin_out[i])
            target = swin_out[i - 1] if i > 0 else x
            _, _, emb_d, emb_h, emb_w = target.shape
            upscaled = upscaled[:, :, :emb_d, :emb_h, :emb_w]
